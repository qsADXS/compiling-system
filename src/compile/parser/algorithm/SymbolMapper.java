        ...
        switch (token.type) {
            ...
            case RESERVED: // 关键字
                switch (token.value) {
                    ...
                    // 基本类型关键字，如果 Lexer 将它们作为 RESERVED 类型 Token 输出，
                    // 并且 Token.parseSpecific() 没有将它们的 TokenType 更改为 TYPE，
                    // 则在此处处理。

                    // 其他关键字 (如 class, public, return 等，如果文法不支持) 应当映射为错误
                    // 或有它们自己的 SymbolType（如果文法支持）
                    default:
                        // 对于未明确处理的保留字，返回错误
                        // 这有助于发现文法和映射器之间不匹配的地方
                        System.err.println("SymbolMapper: Unhandled RESERVED keyword: '" + token.value + "' at line " + token.line + ":" + token.pos);
                        return SymbolType.T_ERROR;
                }
